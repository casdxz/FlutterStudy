...
![LandingOne](https://bearcad.oss-cn-shanghai.aliyuncs.com/Code/LangingOne.png)
...
每一个 task 都值得被精心对待，为它专门换个壁纸，用上好看的背景图和透明卡片。
依旧是 Stack 来层叠，底部容器放背景图，上层用滚动视图，滚动的时候底图不动。
合理划分代码层级和组件，卡片小组件，属性合理设置默认值，提醒自己注意组件的易用性。
思考下那个“网格”效果的实现？（实际上是个Row）
...
![DashboardOne](https://bearcad.oss-cn-shanghai.aliyuncs.com/Code/DashboardOne.png)
