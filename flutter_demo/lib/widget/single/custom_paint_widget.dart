          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Container(
                ...
                child: const Text(
                  '通过CustomPainter进行绘制，可实现一些复杂的自定义绘制组件，是 Flutter中自定义组件的灵魂组件。',
                  style: descStyle,
                ),
              ),
              CustomPaint(
...
// pi:圆周率，表示180度。
// 界面尺寸单位是像素
// CustomPainter展示的是静态时点的静态⻚面，如果想要实现动态效果(如动画，手势)需要通过 setState更新
...
class BasicPainter extends CustomPainter {
  ...
  final Paint _paint = Paint()
    ...
    // ..maskFilter = MaskFilter.blur(BlurStyle.inner, 3.0) //模糊遮罩效果 // ..filterQuality = FilterQuality.high //颜色渲染模式的质量
    // ..strokeCap = StrokeCap.round //末端处理 round:圆滑处理(延伸一小截圆形) square:延伸一小截矩形 butt:不延伸
    // ..strokeJoin=StrokeJoin.round //拐⻆类型,仅影响多边形连线 (Canvas.drawPath) round:圆⻆ miter:锐⻆ bevel:斜⻆
  ;//画笔宽度
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 绘制圆⻆矩形
    // fromLTRBR/fromRectAndRadius:最后一个参数是圆⻆半径
    ...
    // canvas可以直接绘制颜色，画布上的所有颜色均会被混合叠加，可实现全局置灰 // canvas.drawColor(Colors.grey, BlendMode.color);
    // 绘制图像
    // 注意这里的image是ui包里的，而非material包。留意image生成方式
    // offset:图像左上⻆点
    // canvas.drawImage(image, Offset(200, 250), _paint);
    // drawImageRect(Image image, Rect src, Rect dst, Paint paint) // src:截取图像显示部分，dst:确定图像显示区域
    // src确定的部分会在dst确定的范围内平铺
    canvas.drawImage(image, const Offset(260, 0), _paint);
    ...
    // 绘制路径
    // moveTo起始点(没有默认起始点是0，0)，lineTo是途径点(只有1个就是线段) close 为闭合图像
    _paint
    ...
    //reset:取消之前对path的设置,重新设置参数，否则界面上所有已绘制过的path都会共用 _paint新设置的参数
    path.reset();
    _paint.color = Colors.pinkAccent;
    // 绘制⻉塞尔曲线
    //cubicTo(double x1, double y1, double x2, double y2, double x3, double y3)
    //定点(x3,y3)，控制点(x1,y1)和(x2, y2)。 //调用cubicTo前一般先moveTo(x0,y0)确认起始点。
    //起点(x0,y0)，终点(x3,y3), //(x1,y1)指示起点切线方向，(x2,y2)指示终点切线方向。(x1,y1)和(x2,y2)要在绘制路径范围内。
    path.reset();
    //确定桃心顶部中间点
    path.moveTo(200, 450);
    //画桃心左半边
  ...
  // 是否需要重绘。通常在当前实例和旧实例属性不一致时返回true @override
